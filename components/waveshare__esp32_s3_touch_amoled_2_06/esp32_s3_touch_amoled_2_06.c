...
{
    // 1. Definimos o tamanho fixo da fatia (40 linhas)
    int buffer_size = BSP_LCD_H_RES * 40; 
    
    // 2. Limitamos o DMA do hardware a esse tamanho exato
    const bsp_display_config_t disp_config = {
        .max_transfer_sz = buffer_size * (BSP_LCD_BITS_PER_PIXEL / 8),
    };

    BSP_ERROR_CHECK_RETURN_NULL(bsp_display_new(&disp_config, &panel_handle, &io_handle));

    // 3. Configuramos a porta do LVGL
    const lvgl_port_display_cfg_t disp_cfg = {
        .io_handle = io_handle,
        .panel_handle = panel_handle,
        .buffer_size = buffer_size, // Passamos a nossa variável limpa aqui
...
lv_display_t *bsp_display_start(void)
{
    bsp_display_cfg_t cfg = {
        ...
        .buffer_size = BSP_LCD_H_RES * 40, // Crava o LVGL nas mesmas 40 linhas
        .double_buffer = false, // Desliga o duplo buffer para poupar RAM
        .flags = {
            .buff_dma = true,    // Habilita alocação na SRAM Interna
            .buff_spiram = false, // Remove o LVGL da PSRAM
        }};
